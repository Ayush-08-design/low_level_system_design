...
class LogRecord:
    def __init__(self , logger_name : str , level : Level , msg : str , metadata : dict = None):
...
class AsyncDispatcher:
    def __init__(self , handlers , queue_size = 1000 , batch_size = 50 , flush_interval = 0.5 , drop_policy = "drop_oldest"):
        ...
        self._worker = threading.Thread(target = self._run , name = 'LoggerWorker' , daemon = True)
        self._dropped = 0
    ...
    def enqueue(self , record : LogRecord):
        try:
            if self.drop_policy == 'drop_oldest':
                ...
                self.queue.put(record , block = False , timeout = 0.1)
                return True
        except Exception:
            self._dropped += 1
            return False
...
class Logger:
    ...
    def debug(self , msg , **meta):
        self._log(Level.DEBUG , msg , **meta)
        
    def info(self , msg , **meta):
        self._log(Level.INFO , msg , **meta)
            
    def warning(self , msg , **meta):
        self._log(Level.WARNING , msg , **meta)
            
    def error(self , msg , **meta):
        self._log(Level.ERROR , msg , **meta)
            
    def critical(self , msg , **meta):
            self._log(Level.CRITICAL , msg , **meta)
            
            
            
# testing
if __name__ == '__main__':
    fmt = Formatter("{asctime} [{level}] {name} : {msg}")
    ...
    dispatcher = AsyncDispatcher([console , fileh] , queue_size=200 , batch_size=20 , flush_interval=0.2 , drop_policy='')
    dispatcher.start()
    ...
    def worker(thread_id):
        for i in range(50):
            app_logger.info(f'message {i} from thread {thread_id}' , thread_id = thread_id , i = 1)
            time.sleep(0.01)
